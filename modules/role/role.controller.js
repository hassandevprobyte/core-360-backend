...
// @desc    Get roles with pagination
// @route   GET /api/v1/roles
// @access  Private
exports.getRolesWithPagination = asyncHandler(async (req, res) => {
  const payload = {
    filters: req.filters,
  ...
  const data = await roleService.getRolesWithPagination(payload);

  res.status(200).json(data);
});

// @desc    Get all resources
// @route   GET /api/v1/roles/resources
// @access  private
exports.getAllResources = asyncHandler(async (req, res) => {
  const data = roleService.getAllResources();

  res.status(200).json(data);
});

// @desc    Get all scopes
// @route   GET /api/v1/roles/scopes
// @access  private
exports.getAllScopes = asyncHandler(async (req, res) => {
  const data = roleService.getAllScopes();
